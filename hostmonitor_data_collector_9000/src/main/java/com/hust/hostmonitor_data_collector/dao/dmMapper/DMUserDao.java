...
@Mapper
public interface DMUserDao {
    // user
    @Select("set @new_id=0;\n " +
            "call p_getId(@new_id);\n " +
            "set @date=now();\n " +
            "INSERT INTO storagedevicemonitor.UserTable values(@new_id,#{user_name},#{user_password},0,0,@date,'','',0,0);\n " +
            "select @new_id; ")
    ...
    @Select("select * from storagedevicemonitor.UserTable")
    List<SystemUser> getUsers();

    @Select("UPDATE storagedevicemonitor.UserTable " +
