...
import com.hust.hostmonitor_client.utils.Entity.KylinEntity.*;
...
public class KySampler implements Sampler{
    ...
    private void dataObjectInitialization(){
        ...
        int diskNumber= CommandSampler.getDiskStoreSize();
        int i=0;
        ...
        int gpuNumber= CommandSampler.getGraphicsCardSize();
    ...
    public void hardWareSample() {
        ...
        //主机名
        dataObject.put("hostName", CommandSampler.getHostName());
        //操作系统部分 cat /proc/version
        dataObject.put("osName", CommandSampler.getOSName());
        //CPU部分 cat /proc/cpuinfo |grep cpu
        dataObject.getJSONArray("cpuInfoList").getJSONObject(0).put("cpuName", CommandSampler.getCPUName());
        //磁盘
        List<KylinDiskStore> hwDiskStoreList= CommandSampler.getDiskStores();
        ...
        for(i=0;i<hwDiskStoreList.size();i++){
            ...
            dataObject.getJSONArray("diskInfoList").getJSONObject(i).put("diskTotalSize",FormatUtils.doubleTo2bits_double((tempDiskStore.getSize()*1.0/1024/1024/1024)));
        ...
        dataObject.put("allDiskTotalFreeSize",FormatUtils.doubleTo2bits_double(totalsize*1.0/1024/1024/1024));
        //GPU
        List<KylinGPU> graphicsCards = CommandSampler.getGraphicsCardsFromLspci();
        ...
        List<KylinNetworkIF> networkIFList= CommandSampler.getNetworkIFs();
    ...
    public void periodSample(int period, boolean isTheFirstTimeToSample) {
        ...
        for(int j=0;j<DiskStoreList.size();j++){
            ...
            double singleTotalSize=dataObject.getJSONArray("diskInfoList").getJSONObject(i).getDouble("diskTotalSize");
            double singleUsedSize=tempDiskInfo.diskFSUsageAmount*1.0f/1024/1024;
            totalUsedSize+=singleUsedSize;
            dataObject.getJSONArray("diskInfoList").getJSONObject(i).put("diskTotalFreeSize",FormatUtils.doubleTo2bits_double(singleTotalSize-singleUsedSize));
        ...
        double totalsize=dataObject.getDouble("allDiskTotalSize");
        dataObject.put("diskCapacityTotalUsage",FormatUtils.doubleTo2bits_double(totalsize-totalUsedSize));
        //网络速率计算
    ...
    public void processInfoSample(int period, int processFrequency) {
        ...
        processesList= CommandSampler.getProcesses();
